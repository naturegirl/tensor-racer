...
class DataSet(object):
    # TODO: add boolean: shuffle? greyscale?
...
def load_images(data_dir):
    ...
    num_rows = len(labels)
    images = [read_image(os.path.join(data_dir, fpath)) for _, fpath, _ in files]
    matrix = np.concatenate(images).reshape(num_rows, 30*30*3)
    return labels, matrix

def read_image(path, resize=True, size=30, rgb=True):
